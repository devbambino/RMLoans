    ...
    const NavLink = ({ href, children, onClick }: { href: string, children: React.ReactNode, onClick?: () => void }) => {
        ...
        return (
            ...
            >
                ...
                {isActive && (
                    <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-0.5 bg-[#50e2c3] shadow-[0_0_10px_#50e2c3] rounded-full"></span>
                )}
            </Link>
        );
    };
        ...
        <div className={`flex items-center ${isMobile ? 'flex-col items-start w-full gap-3' : 'gap-3'}`}>
            ...
            >
                <div className="relative px-4 py-1.5 rounded-full bg-black/80 backdrop-blur-md flex items-center gap-2 transition-all group-hover:bg-black/90">
                    ...
                    {!copied && (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-gray-500 group-hover:text-[#50e2c3] opacity-0 group-hover:opacity-100 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor">
