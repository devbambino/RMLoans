/**
 * Morpho Vault + Market PoC Frontend Logic
 * 
...
const CONFIG = {
    mockMXNB: "0x", // From deploy.ts
    waUSDC: "0x",   // From deploy.ts
    fixedPriceOracle: "0x", // From deploy.ts
    
    // Base Sepolia (do not change)
    usdc: "0xba50cd2a20f6da35d788639e581bca8d0b5d4d5f",
    vUSDC: "0x10f1a9d11cdf50041f3f8cb7191cbe2f31750acc",
    morphoVault: "0x7B4eb56E7CD4eFc5c4D044DBC3917eB21f3d5dAE",
...
async function refreshAllBalances() {
    ...
    try {
        ...
        // MXNB Balance
        const mxnb = new ethers.Contract(CONFIG.mockMXNB, ERC20_ABI, provider);
        const mxnbBalance = await mxnb.balanceOf(userAddress);
        document.getElementById("mxnbBalance").textContent = 
            ethers.formatUnits(mxnbBalance, 6).substring(0, 10);
...
async function approveAndSupplyUsdc() {
    ...
    try {
        ...
        addLog(`Starting: Supply ${ethers.formatUnits(amount, 6)} USDC to Morpho`, "info");
        setStatus("vaultStatus", "⏳ Approving USDC...", "warning");

        const usdc = new ethers.Contract(CONFIG.usdc, ERC20_ABI, signer);
        const approveTx = await usdc.approve(CONFIG.morphoVault, amount);
        await approveTx.wait();
        addLog(`✓ Approval confirmed: ${approveTx.hash}`, "success");

        setStatus("vaultStatus", "⏳ Supplying to Vault...", "warning");
        const morphoVault = new ethers.Contract(CONFIG.morphoVault, AAVE_POOL_ABI, signer);
        const supplyTx = await morphoVault.supply(CONFIG.usdc, amount, userAddress, 0);
        await supplyTx.wait();
        addLog(`✓ Supply confirmed: ${supplyTx.hash}`, "success");

        setStatus("vaultStatus", "✓ USDC supplied to Vault", "success");
...
async function withdrawFromMorpho() {
    ...
    try {
        ...
        const vUsdc = new ethers.Contract(CONFIG.vUSDC, ERC20_ABI, provider);
        const balance = await vUsdc.balanceOf(userAddress);
        
        addLog(`Starting: Withdraw ${ethers.formatUnits(balance, 6)} vUSDC from Vault`, "info");
        setStatus("vaultStatus", "⏳ Withdrawing...", "warning");

        const morphoVault = new ethers.Contract(CONFIG.morphoVault, AAVE_POOL_ABI, signer);
        const withdrawTx = await morphoVault.withdraw(CONFIG.usdc, balance, userAddress);
        await withdrawTx.wait();
        addLog(`✓ Withdrawal confirmed: ${withdrawTx.hash}`, "success");

        setStatus("vaultStatus", "✓ Withdrawn from Vault", "success");
        updateWorkflowStep(8, "completed");
        
        await refreshAllBalances();
    } catch (error) {
        addLog(`❌ Withdrawal failed: ${error.message}`, "error");
        setStatus("vaultStatus", `Error: ${error.message}`, "error");
    }
}
...
async function wrapAusdcToWausdc() {
    ...
    try {
        ...
        if (balance === 0n) {
            alert("No vUSDC balance to wrap");
            return;
        }

        addLog(`Starting: Wrap ${ethers.formatUnits(balance, 6)} vUSDC to WaUSDC`, "info");
        setStatus("wausdcStatus", "⏳ Approving vUSDC...", "warning");

        const aUsdcSigner = new ethers.Contract(CONFIG.vUSDC, ERC20_ABI, signer);
        ...
        setStatus("wausdcStatus", "✓ vUSDC wrapped to WaUSDC", "success");
...
async function unwrapWausdcToAusdc() {
    ...
    try {
        ...
        addLog(`Starting: Unwrap ${ethers.formatUnits(balance, 6)} WaUSDC to vUSDC`, "info");
        ...
        setStatus("wausdcStatus", "✓ WaUSDC unwrapped to vUSDC", "success");
...
// ============================================================================
// MORPHO MARKET FUNCTIONS
// ============================================================================

function getMarketParams() {
    return [
        CONFIG.mockMXNB,        // loanToken
...
async function borrowFromMorpho() {
    ...
    try {
        ...
        addLog(`Starting: Borrow ${ethers.formatUnits(amount, 6)} MXNB`, "info");
        ...
        setStatus("morphoStatus", "✓ Borrowed MXNB from Morpho", "success");
...
async function repayMorphoLoan() {
    ...
    try {
        ...
        const mxnb = new ethers.Contract(CONFIG.mockMXNB, ERC20_ABI, provider);
        const balance = await mxnb.balanceOf(userAddress);
        
        if (balance === 0n) {
            alert("No mxnb balance to repay");
            return;
        }

        addLog(`Starting: Repay ${ethers.formatUnits(balance, 6)} MXNB`, "info");
        setStatus("morphoStatus", "⏳ Approving MXNB...", "warning");

        const mxnbSigner = new ethers.Contract(CONFIG.mockMXNB, ERC20_ABI, signer);
        const approveTx = await mxnbSigner.approve(CONFIG.morphoBlue, balance);
        await approveTx.wait();
        addLog(`✓ Approval confirmed: ${approveTx.hash}`, "success");
