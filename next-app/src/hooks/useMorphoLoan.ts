...
const TARGET_LTV = 0.5; // Conservative LTV target for calculation
...
export const useMorphoLoan = () => {
  ...
  const waitForSubsidyIncrease = async (
    ...
    while (retries < 15) {
      const currentBalance =
        await tokenContract.userInterestSubsidyInWmUSDC(userAddress);
      if (currentBalance > initialBalance) return currentBalance;
    ...
    throw new Error(
      "RPC timeout: The network is slow indexing your new subsidy. Please wait a moment and try again.",
    );
  };
  ...
  const executeZale = async (borrowAmountMXNB: string) => {
    ...
    try {
      ...
      const morphoUSDCVault = new ethers.Contract(
      ...
      console.log(
        `Calculated Deposit (Vault-based): ${ethers.formatUnits(depositAmountBN, USDC_DECIMALS)} USDC`,
      );
      ...
      const tx3 = await morphoUSDCVault.approve(
        CONTRACT_ADDRESSES.wmUSDC,
        ethers.MaxUint256,
        { gasLimit: MANUAL_GAS_LIMIT },
      );
      ...
      const wmusdcAllowance = await wmUSDC.allowance(
        userAddress,
        CONTRACT_ADDRESSES.morphoBlue,
      );
      if (wmusdcAllowance < wmusdcBalance) {
        const tx5 = await wmUSDC.approve(
          CONTRACT_ADDRESSES.morphoBlue,
          ethers.MaxUint256,
          { gasLimit: MANUAL_GAS_LIMIT },
        );
      ...
      console.log(
        "Market Params:",
        JSON.stringify(MXNB_MARKET_PARAMS_ARRAY, (key, value) =>
          typeof value === "bigint" ? value.toString() : value,
        ),
      );

      // Verify Allowance specifically for Morpho Blue
      const finalAllowance = await wmUSDC.allowance(
        userAddress,
        CONTRACT_ADDRESSES.morphoBlue,
      );
      ...
      if (finalAllowance < currentWmUSDCBalance) {
        console.warn(
          "Allowance mismatch detected! Attempting emergency approval...",
        );
        const txFix = await wmUSDC.approve(
          CONTRACT_ADDRESSES.morphoBlue,
          ethers.MaxUint256,
          { gasLimit: MANUAL_GAS_LIMIT },
        );
      ...
      if (msg.includes("estimateGas"))
        msg =
          "Gas estimation error. Possible network congestion or insufficient connection.";
      if (msg.includes("allowance"))
        msg = "Permission verification failed. Please try again.";
      if (msg.includes("insufficient collateral"))
        msg =
          "Insufficient collateral. The transaction was reverted by the protocol.";
  ...
  const executeRepayAndWithdraw = async () => {
    ...
    try {
      ...
      const mxnbAllowance = await mxnb.allowance(
        userAddress,
        CONTRACT_ADDRESSES.morphoBlue,
      );
      if (mxnbAllowance < mxnbBalance) {
        const tx1 = await mxnb.approve(
          CONTRACT_ADDRESSES.morphoBlue,
          ethers.MaxUint256,
          { gasLimit: MANUAL_GAS_LIMIT },
        );
      ...
      const initialRawSubsidyUSDC =
        await wmUSDC.userInterestSubsidyInWmUSDC(userAddress);
      console.log(
        `Calculating subsidy in MXNB (${borrowShares.toString()} shares) with INitial Subsidy: ${initialRawSubsidyUSDC} WmUSDC...`,
      );
      const userInterestSubsidyInWmUSDC =
        await wmUSDC.getInterestSubsidy(userAddress);
      ...
      const rawEstimatedSubsidyUSDC =
        await wmUSDC.userInterestSubsidyInWmUSDC(userAddress);
      const estimatedSubsidyUSDC = ethers.formatUnits(
        rawEstimatedSubsidyUSDC,
        18,
      );
      const rawEstimatedSubsidyMXNB =
        await wmUSDC.userInterestInMxnb(userAddress);
      const estimatedSubsidyMXNB = ethers.formatUnits(
        rawEstimatedSubsidyMXNB,
        6,
      );
      console.log(
        `User Subsidy: ${estimatedSubsidyUSDC} USDC (${estimatedSubsidyMXNB} MXNB)`,
      );
      ...
      if (collateralShares > 0n) {
        const tx3 = await morpho.withdrawCollateral(
          MXNB_MARKET_PARAMS_ARRAY,
          collateralShares,
          userAddress,
          userAddress,
      ...
      if (wmusdcBalance > 0n) {
        const tx4 = await wmUSDC.redeemWithInterestSubsidy(
          wmusdcBalance,
          userAddress,
          userAddress,
          { gasLimit: MANUAL_GAS_LIMIT },
      ...
      const musdcBalance = await morphoUSDCVault.balanceOf(userAddress);
      if (musdcBalance > 0n) {
        const tx5 = await morphoUSDCVault.redeem(
          musdcBalance,
          userAddress,
          userAddress,
          { gasLimit: MANUAL_GAS_LIMIT },
      ...
      const rawPaidSubsidyUSDC =
        await wmUSDC.userPaidSubsidyInUSDC(userAddress);
      const paidSubsidyUSDC = ethers.formatUnits(rawPaidSubsidyUSDC, 6);
      console.log(
        `Paid Subsidy: ${paidSubsidyUSDC} USDC (${estimatedSubsidyMXNB} MXNB, ${estimatedSubsidyUSDC})`,
      );
      ...
      if (msg.includes("reverted"))
        msg =
          "The transaction was reverted by the protocol. Check your ETH and market liquidity.";
      if (msg.includes("allowance"))
        msg = "Permission verification failed. Please try again.";
      if (msg.includes("insufficient collateral"))
        msg =
