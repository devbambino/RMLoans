...
export const fetchMarketBorrowRate = async (provider: ethers.Provider | ethers.JsonRpcProvider) => {
    ...
    const totalSupplyAssets = Number(ethers.formatUnits(marketDetails.totalSupplyAssets, 6)); // MXNE decimals
    const totalBorrowAssets = Number(ethers.formatUnits(marketDetails.totalBorrowAssets, 6));

    const irmContract = new ethers.Contract(MXNE_MARKET_PARAMS.irm, IRM_ABI, provider);
    const marketTuple = [
